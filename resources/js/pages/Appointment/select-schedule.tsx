...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    },
    {
        ...
        href: '/admin/appointments/select-schedule'
    },
];
...
export default function SelectSchedule() {
    const { schedules, patients } = usePage<SharedData & { 
        schedules: Schedule[]; 
        patients: Patient[];
    }>().props;
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Select Schedule" />
            <div className="p-3 md:p-6">
                <Card className="shadow-sm">                    <CardHeader className="pb-4">
                        <div className='flex justify-between items-start'>
                            <div>
                                <CardTitle className="text-xl md:text-2xl">Select Schedule</CardTitle>
                                <CardDescription>Choose a schedule to create an appointment</CardDescription>
                            </div>
                            ...
                            <table className="w-full min-w-full bg-card">
                                ...
                                <tbody className="bg-card divide-y divide-border">
                                    {filteredSchedules.map((schedule) => (
                                        <Link 
                                            key={schedule.id} 
                                            href={route('admin.appointment.create', { schedule_id: schedule.id })}
                                            className="contents"
                                        >
